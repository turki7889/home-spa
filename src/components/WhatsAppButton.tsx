...
export function WhatsAppButton() {
  return (
    <a
      ...
      className="fixed bottom-6 left-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-[#25D366]/15 transition-all duration-300 hover:scale-110 hover:shadow-xl hover:bg-[#20BD5A] sm:h-16 sm:w-16"
      aria-label="تواصلي معنا عبر واتساب"
      title="تواصلي معنا عبر واتساب"
    >
      <FaWhatsapp className="h-7 w-7 sm:h-8 sm:w-8" aria-hidden="true" />

      {/* Pulse indicator — muted dusty */}
      <span className="absolute -top-1 -right-1 flex h-4 w-4 sm:h-5 sm:w-5">
        ...
        <span className="relative inline-flex h-4 w-4 sm:h-5 sm:w-5 rounded-full bg-accent" />
      </span>
    </a>
  );
}
